...
#define ASSETS "/build/flutter_assets"
...
FlutterApplication::FlutterApplication(char* path) {
  ...
  assets_path = (char*) malloc(strlen(path)+strlen(ASSETS)+1);
  strcpy(assets_path, path);
  strcpy(assets_path+strlen(path), ASSETS);
